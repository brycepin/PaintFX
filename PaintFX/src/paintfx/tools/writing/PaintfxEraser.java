...
/**
 * Implements the drawing shape for a eraser
 * 
 * @author Thomas Chapman
 * @author Bryce Pinder
 */
public class PaintfxEraser extends DrawingInstrument {

   /**
    * Constructor. Sets the tool helper
    * @param th 
    */
   public PaintfxEraser(ToolHelper th) {
      super(th);
   }
      ...
      for(double xc = x-size; xc <= x+size; xc++){
         for(double yc = y-size; yc <= y+size; yc++){
            ...
            if(Math.pow(xc-x,2) + Math.pow(yc-y,2) <= size*size){
               pw.setColor((int)xc,(int)yc, Color.WHITE);
            }
